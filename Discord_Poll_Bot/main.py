...
async def poll(ctx, seconds: int, question: str, *options: str):
    ...
    sort_results = sorted([(value, key) for (key, value) in results.items()], reverse=True)
    ...
    if sort_results:
        ...
        if len(winners) > 1:
            await ctx.send("It's a tie! The most voted options were " + ", ".join(winners) + " with " + str(max_votes) + " votes each.")
        else:
            await ctx.send("The most voted option was " + winners[0] + " with " + str(max_votes) + " votes.")
    else:
