...
def book_get(name, mainres=25, results=5):
    ...
    Results:
                [   0.Book Name,
                    ...
                    6.Book Image Link
                    7.Language]"""
    # making a empty list to store books
    Books = []
    # checking emoji count
    emoji_count = emoji.emoji_count(name)
    if emoji_count > 0:
        # returning error if emoji in name
        return "Error: emoji"
    if name == "":
        # returning error if no book name provided
        return "Error: enter name"
    # adding + for any spaces between name string
    name = name.replace(" ", "+")
    ...
    if "Search string must contain minimum 3 characters.." in bs_html.body:
        # returning error if title short
    ...
    if a > 1:
        ...
        for i in table_rows:
            if counter <= results:
                ...
                # checking if the book's author is in Banned publishers or authors
